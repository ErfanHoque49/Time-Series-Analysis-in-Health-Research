...
subtitle: "Forecasting of Weekly mortality rate in Saskatchewan"
author: "Erfan Hoque \\
...
```{r}
mortality <- read.csv("mortality_model_SK.csv")
```
...
mortality_ts %>% autoplot(Rate) +
  labs(title = "Weekly mortality rate  in Saskatchewan",
...
mortality_ts %>%
  ...
  labs(y = "Rate", title = "Weekly mortality rate in Saskatchewan")
```
...
mortality_ts %>% ACF(Rate) %>% autoplot() +
    labs(title = "Weekly mortality rate in Saskatchewan without differencing",
       y = "Mortality rates per 100,000")
```

```{r, fig.height=4, fig.width=8}
mortality_ts %>% ACF(difference(Rate)) %>% autoplot() +
    labs(title = "Weekly mortality in Saskatchewan with differencing",
       y = "Number of positive cases")
```
...
fit.arima %>% forecast(h = 52) %>%  # h = "1 years"
  autoplot(mortality_ts, level=NULL) +
  labs(y = "Rate", title = "Weekly mortality rate in Saskatchewan using SARIMA")
```
...
fit.ets %>% 
  forecast(h = "1 years") %>% 
  autoplot(mortality_ts, level=NULL)+
  labs(title="Weekly mortality rate in Saskatchewan using ETS", y="Mortality rates per 100,000")
```
...
fit.nn %>% forecast(h="1 years", times=1) %>%
  autoplot(mortality_ts, level=NULL) +
  labs(x = "Week", y = "Mortality rates per 100,000", title = "Weekly mortality rate in Saskatchewan using NNAR")
```
...
fit.ph %>% forecast(h = "1 years") %>% 
  autoplot(mortality_ts, level=NULL) + 
  labs(x = "Week", y = "Mortality rates per 100,000", title = "Weekly mortality rate in Saskatchewan using Prophet")
```
...
mortality_fc %>%
  autoplot(mortality_ts, level = NULL) +
  labs(title = "Weekly mortality reate in Saskatchewan",
       y = "Mortality rates per 100,000") +
  guides(colour = guide_legend(title = "Forecast"))
```
...
on some other predictor, say mean precipitation and temperature of week, trend of the data, any intervention effect (e.g. Covid-19 effect) and so on. We need to assume that morality rate has a linear relationship with these predictors.
...
mortality_ts %>% autoplot(Rate) +
  ...
  annotate("text", x = peak_time + 400, y = 25, label = "COVID-19 Emergency",
...
Let's see the relation between weekly mortality rate and weekly mean temperature and mean precipitation. 
...
Let's fit the time series regression model using covariates/predictors, say COVID-19 emergency intervention effect, trend variable (already built in *fpp3* package as *trend()*)
...
augment(fit.dyn) |>
  ...
  geom_line(aes(y = .fitted, colour = "Fitted")) +
  labs(y = NULL,
    title = "Mortality rate in Saskatchewan"
  ) +
  scale_colour_manual(values=c(Data="black",Fitted="#D55E00")) +
  guides(colour = guide_legend(title = NULL))
